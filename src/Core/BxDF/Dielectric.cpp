...
Spectrum Dielectric::f(const Vector3f& wi, const Vector3f& wo) const
{
    ...
    f32 cos_i = Local::cos_theta(wi), cos_o = Local::cos_theta(wo);
    bool is_reflect = cos_i * cos_o > 0;
    
    f32 eta = is_reflect ? 1 : ((cos_o > 0) ? eta_i / eta_t : eta_t / eta_i);
    ...
    f32 F = Fresnel::dielectric(dot(wi, wh), eta_i, eta_t);
    if(is_reflect)
    {
    ...
    {
        f32 denom = pow2((dot(wo, wh)) + dot(wi, wh) / eta) * cos_i * cos_o;
...
f32 Dielectric::pdf(const Vector3f& wi, const Vector3f& wo) const
{
    ...
    {
        f32 denom = pow2((dot(wo, wh)) + dot(wi, wh) / eta);
        f32 dwh_dwi = std::abs(dot(wo, wh)) / denom;
...
std::optional<bxdf_sample> Dielectric::sample_f(const Vector3f& wi, const Point2f& p) const
{
    ...
    {
        Vector3f wh = GGX.sample_wm(wi, p);
        ...
        else
        {
            ...
            const std::optional<Hinae::Vector3f> is_refract = refract(-wi, wh, ref_idx);
            // total internal reflection
            if(!is_refract) return {};
            ...
            f32 denom = pow2((dot(wo, wh)) + dot(wi, wh) / ref_idx);
            f32 dwh_dwi = std::abs(dot(wo, wh)) / denom;
